...
angular.module('claire.controllers').controller('ClaireCtrl', ["$scope", "$location", "$timeout", "$analytics", "DrugInfo", "Trends",
    function ($scope, $location, $timeout, $analytics, DrugInfo, Trends) {
        ...
        if ($scope.isDetailsPage) {
            ...
            $('.show').on("click", function () {
                ...
                $analytics.eventTrack('Show More/Less', {category: 'Enrichment', label: 'Show More/Less'});
            });

            $('.graph-info').on("click", function () {
                $('.modal').addClass('on');
                $analytics.eventTrack('Graph Info', {category: 'Enrichment', label: 'Graph Info'});
            });
        ...
        if ($scope.isDetailsPage) {
            ...
            DrugInfo.getChart($scope.drug.selected, 200).then(function (response) {
                ...
                chartData.push({
                    data: peaks,
                    points: {
                        show: true, radius: 11, lineWidth: 0, symbol: function (ctx, x, y, r, shadow) {
                            var image = new Image();
